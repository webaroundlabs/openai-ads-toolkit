...
final class EventBuilder
{
    ...
    private function user(array $options): UserData
    {
        ...
        $accepted = $this->acceptableIdentity([
            'email' => $string('email'),
            'phone' => $string('phone'),
    ...
    /**
     * Keep the identity fields the core accepts and drop the ones it refuses.
     *
     * The core is strict about identity, and rightly so: a phone number with an
     * extension welded on hashes to somebody who does not exist. But here the
     * values come from a form a visitor filled in, which is untrusted input
     * rather than a programming error - and losing a paid order because a
     * customer typed "ext. 12" after their number is the wrong trade. Each
     * field is offered to the core alone, so one bad value costs only itself
     * and the conversion is still reported with whatever else matched.
     *
     * The value never reaches the hook; only the field name and the core's
     * message, which names no identifier either.
     *
     * @param array<string, string|null> $candidates UserData parameter name => raw value
     *
     * @return array<string, string>
     */
    private function acceptableIdentity(array $candidates): array
    {
        $accepted = [];

        foreach ($candidates as $parameter => $value) {
            if ($value === null) {
                continue;
            }

            try {
                UserData::create(...[$parameter => $value]);
            } catch (InvalidArgument $e) {
                \do_action('openai_ads_identity_field_dropped', $parameter, $e->getMessage());
