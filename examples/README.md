...
| [`php/image-tag-in-an-email.php`](php/image-tag-in-an-email.php) | The no-JavaScript channel, and the two things it refuses to carry. Prints markup; sends nothing. |
...
parses, and they do not pin dependency versions that would go stale. The two
PHP ones will actually run from a monorepo checkout once
`packages/php/vendor` exists, and neither sends a real conversion.
